...
- Skill Tuning: a Foundation-only typed draft remains separate from the committed snapshot. Every edit constructs and validates a complete Runtime + Champion candidate and all four skill descriptors. Invalid edits keep the previous fully valid preview candidate. Preview Only updates pure geometry and a draw-only overlay with zero Dispatcher calls. Its permanent labeled inspector scrolls independently from fixed Save, Revert, and Defaults actions. Live Cast opens a Lifecycle-managed gate for only the selected candidate skill, that descriptor's Coalescer, and the shared Cancel Zone while Battle movement, Attack, unselected skills, and unselected Coalescers remain disabled.
...
Aim presentation reuses `MobaAimGeometry`, `MobaPointCastGeometry`, `MobaPointResponse` and `MobaGameCanvas`. Production orchestration and Preview share `MobaSkillMeaningfulDragForDescriptor`, including its strict wheel-radius times deadzone boundary and named Directional fallback. Returning below or exactly to that boundary restores the configured default target. Preview release consumes its final finite endpoint before ending local ownership. Directional outlines sample `MobaAimTargetForDirection`. Point minimum and maximum outlines separately sample `MobaPointCastTargetForDirection` at ratios zero and one, so a ray selecting any zero minimum radius correctly degenerates to the hero anchor without invalidating the remaining samples. Both paths use canvas clamping rather than a symmetric UIKit ellipse. `MobaAimPreviewMapGamePointToVideoRect` maps inclusive canvas endpoints into the actual Runtime Aspect Fit `videoRect`, including black-bar offsets. Safe-area normalized layout coordinates are not inputs to aim geometry.

Skill Tuning persistence uses an exact-base Repository candidate. Restore Defaults patches only the Issue #21 managed Runtime and compatible canonical Champion aim fields into the draft. Champion identity, metadata, input actions, cast types, unknown fields, future skills, and unrelated Runtime fields remain unchanged. The patched draft is fully decoded and candidate-built before it can replace the last valid preview. Save validates Runtime and current Champion with the unchanged Input and Layout models, prebuilds the runtime and complete Q/W/E/R package, enters profile reload, atomically writes Runtime then Champion, conditionally commits the snapshot and installs the prepared package. Failure restores both previous byte sequences and the previous snapshot identity. Input and Layout are never written.
...
`MobaProfileTransferService` is the Foundation-only preview and export boundary. It first checks complete strong structural signatures. Unknown root or nested fields cannot create another match. Only when no strong signature matches does a narrow ID and core-field fallback route malformed content to one Decoder for its field-specific error. Two complete signatures are ambiguous at `$`. The selected existing Decoder remains the only schema authority. The immutable `MobaProfileImportPlan` captures source bytes, destination, active raw bytes, base snapshot identity, summary, runtime, and a complete Q/W/E/R package. Preview never writes the Store or changes active state.

`MobaProfileImportTransaction` owns confirmed replacement. It rechecks snapshot identity and raw active bytes, enters the existing profile-reload Lifecycle boundary, prepares a reversible production installation, creates a complete backup, atomically writes the destination, commits the Repository candidate, and commits the prepared installation. Prepare requires suspended, disabled, neutral and token-free Movement and Attack state and captures Champion selection, catalog, runtime, package, Views, participants, input mapping, presentation and active path. Commit uses preflighted non-failing controller setters. Failure rolls the installer back before Repository identity and target bytes. Rollback errors retain separate installer, Repository and storage causes. A newly created Champion file is removed, and a backup that fails before its manifest is complete is discarded.

`MobaProfileTransferViewController` owns only buttons, system document pickers, coordinated security-scoped reads, confirmation, status presentation, and temporary export cleanup. Explicit Close and adaptive Form Sheet dismissal converge on one idempotent close boundary that clears pending plans and temporary exports before notifying Coordinator. Picker cancellation does not close its parent manager. Coordinator transitions to UI mode before presenting it, so the Lifecycle closes the Battle input gate and releases tracked input. Background, rotation, disconnect, stop, disappearance, and feature disable dismiss pending transfer UI without automatically returning to Battle.

Champion imports never use the external file name. Existing identifiers containing only ASCII letters, digits, hyphen and underscore retain `champions/<id>.json`. Other schema-valid UTF-8 identifiers use base64url beneath the separate `champions/encoded/id-<encoded>.json` namespace. This keeps the original model and catalog ID while preventing traversal and collisions with readable paths. Successful installation updates the selection catalog and performs one prepared runtime/control replacement. The Store remains a safe byte-I/O layer, the Repository remains the exact-base snapshot owner, and no transfer UI calls Dispatcher or a Decoder directly.
