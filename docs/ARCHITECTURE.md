...
All public read and write paths are relative to the standardized `Application Support/MOBA` root. Absolute paths, empty or dot components, parent traversal, the root itself, and resolved paths outside the root are rejected. Writes use Foundation atomic replacement in the destination file system. `replaceExisting = NO` preserves an existing file, while `replaceExisting = YES` permits atomic replacement without a delete-then-write window. Reads and writes accept arbitrary bytes and deliberately perform no schema inspection.
