...
`MobaControlLayoutPresentation` is the shared visual override boundary for Movement, Attack, and Skill views. Cancel Zone has a specialized presentation value. Final gameplay alpha is `perStateOpacity * globalOpacityMultiplier`, clamped to 0...1. Toolbar, champion selector, editor UI, diagnostics, and system UI do not use this multiplier. Opacity zero remains visually transparent but does not change `interactionEnabled`.
