...
Aim presentation reuses `MobaAimGeometry`, `MobaPointCastGeometry`, `MobaPointResponse` and `MobaGameCanvas`. Production orchestration and Preview share `MobaSkillMeaningfulDragForDescriptor`, including its strict wheel-radius times deadzone boundary and named Directional fallback. Returning below or exactly to that boundary restores the configured default target. Preview release consumes its final finite endpoint before ending local ownership. Asymmetric range outlines are sampled through production ray geometry and canvas clamping rather than approximated with a symmetric UIKit ellipse. `MobaAimPreviewMapGamePointToVideoRect` maps inclusive canvas endpoints into the actual Runtime Aspect Fit `videoRect`, including black-bar offsets. Safe-area normalized layout coordinates are not inputs to aim geometry.
